...
# snow cover, etc. are characters which indicates there is something that's not a number somewhere in those columns...
```
...
```{r}
snowsurvey_csv %>% 
  count(Snow_cover) # get a count of all values that exist in the column

# how do we work with the "-" and "." values?
```
...
species_long <- species_csv %>% 
  pivot_longer(
    cols = !c(Year, Site, Date, Jdate, Num_observers, All_obs_reported, Observer_hours), # don't collapse these columns
    names_to = "species",
...
species_presence <- species_presence %>% 
  select(-c(Num_observers, Observer_hours, All_obs_reported))
```
